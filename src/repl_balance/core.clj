(ns repl-balance.core
  (:refer-clojure :exclude [read-line])
  (:require
   [clojure.string :as string]
   [repl-balance.commands :as commands]
   [repl-balance.io.callback-reader]
...
(defmacro ensure-terminal
  ...
  [& body]
  `(binding [repl-balance.jline-api/*terminal*
             (or repl-balance.jline-api/*terminal* (repl-balance.jline-api/create-terminal))]
     ~@body))
...
(defmacro with-line-reader
  ...
  repl-balance.jline-api/*line-reader* binding as an alternative in
  source that the repl-balance.core/read-line function reads from.

  Example:
  (require '[repl-balance.core :as rebel])
  ...
  (rebel/with-line-reader
    ...
    ;; this is optional
    (binding [*out* (repl-balance.jline-api/safe-terminal-writer)]
      (clojure.main/repl
  ...
  [line-reader & body]
  `(ensure-terminal
    (repl-balance.utils/load-slow-deps!)
...
(defn read-line-opts
  "Like read-line, but allows overriding of the LineReader prompt, buffer, and mask parameters.
   ...
   :mask
     ...
     Might do crazy stuff with repl-balance, use with caution.
...
(defmacro with-readline-in
  ...
  (with-readline-in
    (repl-balance.clojure.line-reader/create
      (repl-balance.clojure.service.local/create {:prompt clojure.main/repl-prompt} ))
   (clojure.main/repl :prompt (fn[])))"
  [line-reader & body]
  `(with-line-reader ~line-reader
     (binding [*in* (clojure.lang.LineNumberingPushbackReader.
                     (repl-balance.io.callback-reader/callback-reader
                      stream-read-line))]
       ~@body)))
